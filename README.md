...
The `Private MTProto Artifact Publication` workflow runs the ordinary repository and deployment-stack checks first, then stages the verified bundle before a minimal publisher job uploads the bundle and `mtproto-target.json` together as a GitHub Actions artifact named with the source commit and artifact digest. Manual publication runs only when the selected ref is `refs/heads/master` or an exact release tag listed with its reviewed commit in `ci/private-artifact-reviewed-release-refs.json`; an unreviewed branch or tag is rejected before dependency installation and publication. It uses the reviewed public target in `ci/private-mtproto-target.json`; changing the workflow values, public-key bytes, target, or source commit without updating the attestation fails before upload. After downloading a unit, verify its sidecar digest and bundle audit with `MTPROTO_TARGET_MODE=private pnpm run check-bundle -- <artifact-directory>`; the publication job additionally verifies the attested source commit and target before upload.
